    ...
    <div class="sort">
        <div class="bd">
            <form name="listform">
                ...
                <!--<a title="销量" class="curr" rel="nofollow" @click="sortType('-sold_num')"><span :class="{search_DESC: cur==='-sold_num'},{search_ASC: cur ==='sold_num'}">销量</span></a>-->
                <!--<a title="价格" class="curr" rel="nofollow" @click="sortType('-shop_price')"><span :class="{search_DESC: cur==='-shop_price'},{search_ASC: cur ==='shop_price'}">价格</span></a>-->
...
<script>
  export default {
    ...
    methods: {
        // 排序方式
        sortType (type) {
          if (this.old_search === type){
            this.cur = type.substring(1, type.length);
            this.old_search = type.substring(1, type.length);
